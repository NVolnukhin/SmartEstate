...
public class SmtpEmailService : IEmailService, IDisposable
{
    ...
    public async Task SendPasswordRecoveryEmailAsync(string email, string token)
    {
        ...
        var recoveryLink = $"http://smartestate/pages/password_recovery.html?token={token}";
    ...
    {
        var subject = "Смена почты";

        await SendEmailAsync(email, subject, EmailTemplates.GetChangeEmailTemplate(userName));
    }
