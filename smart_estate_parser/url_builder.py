from smart_estate_parser.constants import *
...
class URLBuilder:
    ...
    def add_district(self, district):
        self.url += DISTRICT_PATH.format(district)
    ...
    def add_additional_settings(self, additional_settings):
        ...
        if "metro" in additional_settings and "metro_station" in additional_settings:
            metro_ids = []
            for metro_city, metro_station_name in zip(additional_settings["metro"],
                                                      additional_settings["metro_station"]):
                if metro_city in METRO_STATIONS:
                    for station_info in METRO_STATIONS[metro_city]:
                        if station_info[0] == metro_station_name:
                            metro_ids.append(str(station_info[1]))
            if metro_ids:
                for position, id in enumerate(metro_ids):
                    self.url += f"&metro%5B{position}%5D={id}"
