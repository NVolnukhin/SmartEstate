    ...
    def add_additional_settings(self, additional_settings):
            ...
            if "metro_station" in additional_settings.keys():
                if additional_settings["metro"] in METRO_STATIONS.keys():
                    ...
                    for metro_station, metro_id in METRO_STATIONS[additional_settings["metro"]]:
                        for station in additional_settings["metro_station"]:
                            if station in metro_station:
                                added_stations.append(METRO_ID_PATH.format(i, metro_id))
                                i += 1
                    self.url += ''.join(added_stations)
        ...
        if "district" in additional_settings:
            try:
                added_districts = []
                i = 0
                for district_name, district_id in MOSCOW_DISTRICTS.items():
